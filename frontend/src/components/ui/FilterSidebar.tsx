...
export default function FilterSidebar() {
  ...
  const [isOpen, setIsOpen] = useState(false) // Default to closed to show off the label
  ...
  return (
    <>
      ...
      <motion.div
        initial={{ x: 300 }}
        animate={{ x: isOpen ? 0 : 300 }} // Slide animation
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="relative group">
              ...
              <input
                type="text"
                value={filters.geoSearchQuery}
                onChange={(e) => updateFilters({ geoSearchQuery: e.target.value })}
                placeholder="Search area..."
                className="w-full pl-9 pr-3 py-2.5 bg-slate-900/50 border border-slate-800 text-slate-300 text-xs rounded-lg focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none placeholder:text-slate-600 transition-all"
              />
            </div>
          </div>
